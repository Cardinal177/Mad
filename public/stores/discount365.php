...
<head>
    ...
    <title>365discount Tilbud</title>
    <style>
        :root {
            --accent: #2e7d32;
            --accent-dark: #1f5f24;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .site-header { position: sticky; top: 0; z-index: 100; background: #2e7d32; color: white; box-shadow: 0 2px 12px rgba(0,0,0,0.18); }
        ...
        .store-hero { background: linear-gradient(135deg,#2e7d32 0%,#1f5f24 100%); color: white; padding: 28px 20px 24px; }
        ...
        .spinner { width: 36px; height: 36px; border: 3px solid rgba(46,125,50,0.15); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.75s linear infinite; }
    ...
    <div class="header-inner">
        ...
        <div class="header-brand">
            <svg viewBox="0 0 210 38" xmlns="http://www.w3.org/2000/svg">
                <text y="32" font-family="Arial Black,Arial,sans-serif" font-size="36" font-weight="900" fill="white">365discount</text>
            </svg>
        </div>
    ...
    <div class="hero-inner">
        <div class="store-logo-box">
            <svg viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg" width="72" height="72">
                <rect x="0" y="0" width="80" height="80" rx="14" fill="#2e7d32"/>
                <text x="40" y="43" font-family="Arial Black,Arial,sans-serif" font-size="20" font-weight="900" fill="#fff" text-anchor="middle" dominant-baseline="middle">365</text>
            </svg>
        </div>
        <div class="hero-text">
            <h1>365discount Tilbud</h1>
...
<div class="controls-wrap">
    <div class="controls-inner">
        <div class="search-box">
            ...
            <input type="text" id="searchInput" class="search-input" placeholder="Søg i 365discount tilbud…">
        </div>
...
(function () {
    ...
    async function load() {
        ...
        try {
            ...
            all = (d.items || []).filter(i => String(i.store_name || '').toLowerCase() === String(STORE).toLowerCase());
            const matched = all.filter(i => i.is_catalog_matched).length;
            const validTo = all.find(i => i.valid_to)?.valid_to;
            document.getElementById('heroSub').textContent = all.length + ' tilbud' + (validTo ? ' · Gyldig til ' + shortDate(validTo) : '');
    ...
    function render(items) {
        ...
        box.querySelectorAll('.offer-card').forEach(el => el.addEventListener('click', () => toggle(Number(el.dataset.id))));
    }

    function card(i) {
        const sel = selected.has(i.id);
        const valid = shortDate(i.valid_to);
        const img = i.image_url ? `<img src="${esc(i.image_url)}" alt="" loading="lazy">` : `<div class="no-img">🛍</div>`;
    ...
    function toggle(id) {
        ...
        document.querySelectorAll('.offer-card').forEach(el => el.classList.toggle('selected', selected.has(Number(el.dataset.id))));
    ...
    function toast(msg) {
        ...
        Object.assign(t.style, { position:'fixed', bottom:'80px', left:'50%', transform:'translateX(-50%)', background:'#14231d', color:'white', padding:'11px 20px', borderRadius:'9px', fontSize:'14px', fontWeight:'600', zIndex:'9999', boxShadow:'0 4px 16px rgba(0,0,0,0.25)', transition:'opacity 0.3s', whiteSpace:'nowrap' });
